...
assert(/function stitchMaskPreviewEnabled/.test(source), "tile editor should expose a stitch mask preview toggle state");
...
assert(/stitchedMaskValues/.test(source), "tile editor should preview hardened replace stitch masks");
...
assert(/Stitch mask/.test(source), "tile editor should expose a stitch mask preview toggle");
assert(/Raw masks/.test(source), "tile editor should expose a raw mask preview toggle");
assert(/ttpSmartTileShowMasks/.test(source), "mask overlay visibility should be stored on the editor node instance");
assert(/ttpSmartTilePreviewStitchMask/.test(source), "stitch mask preview should be stored on the editor node instance");
...
assert(/tintedMaskCanvas\(maskImage, color, \{/.test(source), "renderMaskOverlay should pass preview options into mask tinting");
assert(/node\.ttpSmartTileShowMasks = true;/.test(source), "stitch mask preview should auto-enable colored mask overlays");
assert(/Move/.test(source), "tile editor should expose an explicit move mode");
assert(/node\.ttpSmartTilePaintMode = "off";\s*renderEditor\(node\);/s.test(source), "Move should restore tile drag and resize mode");
assert(/pointer-events:" \+ \(paintMode === "off" \? "auto" : "none"\)/.test(source), "tile regions should only block dragging while Brush or Erase is active");
